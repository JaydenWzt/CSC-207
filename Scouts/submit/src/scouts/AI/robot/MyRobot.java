...
public class MyRobot extends ImageComponent implements IRobot {
    ...
    public Move GetNextAction() {
        ...
        if(MyGrid.Get(MyPos).HasArtifact())//if the robot is on an artifact
        {
            TakeArtifact = true;
            TargetArtifact = MyTile.Idol;
        ...
        for(Vector2i tilePos: MyGrid.IndexSet(true))
        {
            ...
            if(MyGrid.Get(tilePos).HasArtifact() && ArtifactsGained.isEmpty())
            {
    ...
    private Move NextMoveToDes(Vector2i destination, boolean explicit)
    {
        ...
        if(Math.abs(dir.X) > Math.abs(dir.Y) || (Math.abs(dir.X) == Math.abs(dir.Y) || MyGrid.Get(choiceX).Cost <= MyGrid.Get(choiceY).Cost))
        {
        ...
        else if(Math.abs(dir.X) < Math.abs(dir.Y) || (Math.abs(dir.X) == Math.abs(dir.Y) || MyGrid.Get(choiceX).Cost > MyGrid.Get(choiceY).Cost))
        {
    ...
    private Move NextMoveToAltar()
    {
        ...
        if(!MyTile.Environment.equals(MyGrid.Get(MyPos.X, MyPos.Y - 1).Environment))
            leftdiffer = true;
        else if(!MyTile.Environment.equals(MyGrid.Get(MyPos.X, MyPos.Y + 1).Environment))
            rightdiffer = true;
        if(!MyTile.Environment.equals(MyGrid.Get(MyPos.X - 1, MyPos.Y).Environment))
            updiffer = true;
        else if(!MyTile.Environment.equals(MyGrid.Get(MyPos.X + 1, MyPos.Y).Environment))
            downdiffer = true;
    ...
    {
        Vector2i up = MyPos.Add(Vector2i.UP);
        Vector2i down = MyPos.Add(Vector2i.DOWN);
        ...
        if(choice == 1)
        {
            if(!MyGrid.IsCellEmpty(MyPos.Add(Vector2i.DOWN)) && MyGrid.Get(MyPos.Add(Vector2i.DOWN)).IsWall())
                choice = 2;
            else
            {
                NextPos = MyPos.Add(Vector2i.DOWN);
        ...
        if(choice == 2)
        {
            if(!MyGrid.IsCellEmpty(MyPos.Add(Vector2i.LEFT)) && MyGrid.Get(MyPos.Add(Vector2i.LEFT)).IsWall())
                choice = 3;
            else
            {
                NextPos = MyPos.Add(Vector2i.LEFT);
        ...
        if(choice == 3)
        {
            if(!MyGrid.IsCellEmpty(MyPos.Add(Vector2i.RIGHT)) && MyGrid.Get(MyPos.Add(Vector2i.RIGHT)).IsWall())
                choice = 0;
            else
            {
                NextPos = MyPos.Add(Vector2i.RIGHT);
    ...
    private void MapUpdate(IGraph<VisionData, Direction> vision)
    {
        ...
        {
            if(visited.contains(ID))//If the vertex has already been visited, skip it
                continue;
            ...
            for(int nextID: GraphAlgorithms.FindPath(vision, 0, ID))//For each vertex in the path from my robot to the vertex
            {
                ...
                if(!visited.contains(nextID))//If the vertex has not been visited, add it to the map
                {
                    visited.add(nextID);
                    MyGrid.Set(vision.GetVertex(nextID), pos);
